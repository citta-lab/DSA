 ...
 *
 *
 * This is Similar to 23. Merge k Sorted Lists where Lists uses DIVIDE and CONQUER but here we are dealing with 
 * Arrays and will use Priority Queue.
 *
 *
 * Ref: https://medium.com/outco/how-to-merge-k-sorted-arrays-c35d87aa298e ( detailed example )
 *
 * Hint:
 * Using Sorting:
 * - Time: O(N*K*log(NK)) Space:O(NK)
 * Using MinHeap :
 * - Time: O(N*K*log(K)) Space:O(NK) 
 * - Use minHeap to track smallest element from all of the arrays. will continue removing smallest number
 * until all the elements are added to the result
 * - Heap needs to know plucked numbers array and it's index ( which array and what index )
 * - Only element index will change in the minheap
 */ 
