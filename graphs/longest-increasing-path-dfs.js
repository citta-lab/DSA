    ...
    for(let i=0; i<matrix.length; i++) {
      for(let j=0; j<matrix[0].length; j++){
          /** update only if we get increment, send -Infinity/-1 as first default parent cell value */
           max = Math.max(max, dfs(i, j, matrix, -Infinity))
      }
    }
...
const dfs = (row, col, array, parentValue) => {
    ...
    maxPath = Math.max(maxPath, dfs(row-1, col, array, currentValue ))
    maxPath = Math.max(maxPath, dfs(row+1, col, array, currentValue ))
    maxPath = Math.max(maxPath, dfs(row, col-1, array, currentValue ))
    maxPath = Math.max(maxPath, dfs(row, col+1, array, currentValue ))
