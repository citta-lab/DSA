...
const findParent = (node, parent) =>{  
    if(node != parent[node]){
        let rootNode = findParent(parent[node], parent);
        parent[node] = rootNode;
    }
    return parent[node];
}

/** flavor one: alternative findParent options */
const findParentTwo = (node, parent) => {
...
/** flavor two: alternative findParent options */
const findParentThree = (node, parent) => {
    if (parent[node] != node) {
